...
void results()
{
    ...
    for(int n=1;n<num_candidates;n++)
        {
            printf("\nhi\n");
            if(kl<candidates[n].number)
            {
                kl=candidates[n].number;
                ki=candidates[n].name;
            }
            if(kl==candidates[n].number)
            {
                count++;
                if(count==2)
                {   flow=1;
                    printf("The are tied.\n For re-elect resultions press y\n for the display of results regardless of tie press n");
                    system("cls");
                    scanf("%c",&ym);
                    if(ym=="y"){
                        printf("Re-election process has been chosen.\n");
                        AdminMode();
                        break;
                    }
                    else{
    ...
    if(flow==0){
        printf("The candidate with the highest number of votes and won the election is %s %d\n",ki,kl);
        printf("For complete results press y");
        system("cls");
        scanf("%c",&xm);
        if(xm=="y")
        {
